import { DataProvider, GetListParams, GetListResult, GetOneParams, GetOneResult, GetManyParams, GetManyResult, Identifier } from 'react-admin';
...
const ordersApiDataProvider: Partial<DataProvider> = {
  getList: async <RecordType extends { id: Identifier } = ApiOrder>(resource: string, params: GetListParams): Promise<GetListResult<RecordType>> => {
    if (resource !== 'orders') {
  ...
  getOne: async <RecordType extends { id: Identifier } = ApiOrder>(resource: string, params: GetOneParams<RecordType>): Promise<GetOneResult<RecordType>> => {
    if (resource !== 'orders') {
      throw new Error(`Unknown resource: ${resource}`);
    }
  ...
  getMany: async <RecordType extends { id: Identifier } = ApiOrder>(resource: string, params: GetManyParams<RecordType>): Promise<GetManyResult<RecordType>> => {
    if (resource !== 'orders') {
      throw new Error(`Unknown resource: ${resource}`);
    }
    ...
    const data: ApiOrderResponse = await response.json();
    const orders = data.items.filter(item => params.ids.includes(item.id.toString()));
